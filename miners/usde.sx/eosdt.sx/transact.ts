...
import { usdeSx, flash, eosdtSx } from "../../../plugins"
...
export async function transact( account: Name, quantity: Asset, base_ext_sym: ExtendedSymbol, quote_ext_sym: ExtendedSymbol ) {
    // calculations
    const rate = await usdeSx.get_calculate_rate( quantity, quote_ext_sym.get_symbol().code() );
    ...
    const actions = [
        ...
        eosdtSx.buymarket( account, quote_ext_sym.get_contract(), rate, base_ext_sym.get_symbol().code() ),
