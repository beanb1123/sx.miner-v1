...
import { vigorSx, flash, stableSx } from "../../../plugins"
...
export async function transact( account: Name, quantity: Asset, base_ext_sym: ExtendedSymbol, quote_ext_sym: ExtendedSymbol ) {
    // calculations
    const { out } = await vigorSx.get_calculate_rate( quantity, quote_ext_sym.get_symbol().code() );
    ...
    const actions = [
        ...
        stableSx.buymarket( account, quote_ext_sym.get_contract(), out, base_ext_sym.get_symbol().code() ),
